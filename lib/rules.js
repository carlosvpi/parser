...
const Conc = module.exports.Conc = (input, config = {}) => {
  ...
  const type = node === null ? 'Expression'
    : node[0] === 'NT' ? 'NT'
    : node[1][2][0] === 'LITERAL' && node[1][2][1] === '*' ? 'Multiplication'
    : node[1][0][0] === 'LITERAL' && node[1][0][1] === '"' ? 'DblQuote'
    : node[1][0][0] === 'LITERAL' && node[1][0][1] === "'" ? 'SglQuote'
    : node[1][0][0] === 'LITERAL' && node[1][0][1] === '/' ? 'Regex'
    : node[1][0][0] === 'LITERAL' && node[1][0][1] === '[' ? 'Option'
    : node[1][0][0] === 'LITERAL' && node[1][0][1] === '{' ? 'Repetition'
    : 1
  const value = type === 'Expression' ? null
    : type === 'NT' ? node[1]
    : type === 'Multiplication' ? [node[1][0][1], node[1][4][1]]
    : type === 'DblQuote' ? node[1][1][1]
    : type === 'SglQuote' ? node[1][1][1]
    : type === 'Regex' ? node[1][1][1]
    : type === 'Option' ? node[1][2]
    : type === 'Repetition' ? node[1][2]
    : 1
...
const Disj = module.exports.Disj = (...inputs) => {
  ...
  return [['Exception', node && node[0], (node && node[1] && node[1][1] && node[1][1][1] && node[1][1][1][2]) || null], rest]
}
...
const Rule = module.exports.Rule = (...inputs) => {
  ...
  const head = node ? node[0] : null
  const body = node ? node[3] : null

  return [['Rule', head, body], rest]
}
